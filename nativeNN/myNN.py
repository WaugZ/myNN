...
class myNN:
    ...
    def __init__(self, num_feature, alpha):
        self.W1 = np.random.randn(5, num_feature) * .01
        self.W2 = np.random.randn(5, 5) * .01
        self.W3 = np.random.randn(3, 5) * .01
        self.W4 = np.random.randn(1, 3) * .01
    ...
    def train(self, train_data, labels, max_iter):
        for i in range(max_iter):
            ...
            '''cost'''
            J = - np.sum(np.multiply(labels, np.log(A4 + 1e-10)) + np.multiply((1 - labels), np.log(1 - A4 + 1e-10))) / self.m
